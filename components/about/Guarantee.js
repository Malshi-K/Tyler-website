...
const GuaranteeSection = () => {
  const cardsData = [
    ...
    },
    {
      ...
      description: {
        type: "paragraph",
        content: "The number of tradespeople and professionals involved in a building project can be a bit daunting. At T. Wilson Builders, Tyler and his team have a strong people-focused culture and strive always to coordinate your project with clarity, thereby reducing the potential for chaos."
      }
    },
    {
    ...
    },
    {
      ...
      description: {
        type: "paragraph",
        content: "At T. Wilson Builders we pride ourselves on being one of Waikato's most recommended building companies. Actions speak louder than words: most of our contracts come to us via word of mouth. Our mission is to ensure you have a great building experience with us."
      }
    },
    {
  ...
  const renderDescription = (description) => {
    switch (description.type) {
      case "paragraph":
        return <p className="text-gray-700 leading-relaxed text-center">{description.content}</p>;
      
      case "paragraphs":
        return description.content.map((paragraph, idx) => (
          <p key={idx} className="text-gray-700 leading-relaxed mb-4 text-center">
            {paragraph}
          </p>
        ));
        ...
        return (
          <div className="text-gray-700 leading-relaxed">
            {description.paragraphs.map((para, idx) => (
              <p key={idx} className="mb-4 text-center">{para}</p>
            ))}
            ...
            {description.footer && (
              <p className="mt-4">{description.footer}</p>
            )}
          </div>
        );
